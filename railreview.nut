...
function CivilAI::ConvertTrack(t, type) {
	   if (AITunnel.IsTunnelTile(t)) { t = AITunnel.GetOtherTunnelEnd(t); AIRail.ConvertRailType(t, t, type); } 
  else if (AIBridge.IsBridgeTile(t)) { t = AIBridge.GetOtherBridgeEnd(t); AIRail.ConvertRailType(t, t, type); } // that was easy enough
								else { AIRail.ConvertRailType(t, t, type); }
return t;
}
...
if (year + 5 <= AIDate.GetYear(AIDate.GetCurrentDate())) { DudRailCon.RemoveItem(town);
...
if (AIRail.IsRailDepotTile(depot) && AITile.GetOwner(depot) == Me) {return depot} else {return null;}
}
...
if (AIRail.GetRailStationDirection(tile) == AIRail.RAILTRACK_NW_SE) {
		xo = 0; yo = 1;  // station is on y axis
	} else {
		xo = 1; yo = 0;  // station is on x axis
	}
...
function CivilAI::DemoTrack(t) {
	   if (AITile.GetOwner(t) != Me) {return t;}
  else if (AIRail.IsLevelCrossingTile(t)) { AIRail.RemoveRailTrack(t, AIRail.RAILTRACK_NE_SW); AIRail.RemoveRailTrack(t, AIRail.RAILTRACK_NW_SE); }
  else if (AITunnel.IsTunnelTile(t)) { t = AITunnel.GetOtherTunnelEnd(t); AITile.DemolishTile (t); } 
  else if (AIBridge.IsBridgeTile(t)) { t = AIBridge.GetOtherBridgeEnd(t); AITile.DemolishTile (t); } // that was easy enough
								else { AITile.DemolishTile (t); }
return t;
}

function CivilAI::CheckJunction(t, dir) {

if (AITunnel.IsTunnelTile(t) || AIBridge.IsBridgeTile(t)) {return false;} // tunnel or bridge, carry on regardless
...
if ((AIRail.GetRailTracks(t) == AIRail.RAILTRACK_NE_SW) ||
	...
	(AIRail.GetRailTracks(t) == AIRail.RAILTRACK_NE_SE)) {return false;} // not a junction, carry on regardless
...
if (dir == 0) {
			if ((AIRail.GetRailTracks(t) == (AIRail.RAILTRACK_NW_SE + AIRail.RAILTRACK_SW_SE)) ||
				...
				(AIRail.GetRailTracks(t) == (AIRail.RAILTRACK_SW_SE + AIRail.RAILTRACK_NE_SE))) {return false;} // a trailing junction which is presumably the start of a passing place. Remove and continue.
				...
				AIRail.RemoveRailTrack(t, AIRail.RAILTRACK_NE_SE);
																								  return true;} // remove our spur tracks and end demolition.
...
if (dir == 1) {
			if ((AIRail.GetRailTracks(t) == (AIRail.RAILTRACK_NW_SE + AIRail.RAILTRACK_NW_SW)) ||
				...
				(AIRail.GetRailTracks(t) == (AIRail.RAILTRACK_NW_SW + AIRail.RAILTRACK_NW_NE))) {return false;} // a trailing junction which is presumably the start of a passing place. Remove and continue.
...
if (dir == 2) {
			if ((AIRail.GetRailTracks(t) == (AIRail.RAILTRACK_NE_SW + AIRail.RAILTRACK_NW_SW)) ||
				...
				(AIRail.GetRailTracks(t) == (AIRail.RAILTRACK_NW_SW + AIRail.RAILTRACK_SW_SE))) {return false;} // a trailing junction which is presumably the start of a passing place. Remove and continue.
...
if (dir == 3) {
			if ((AIRail.GetRailTracks(t) == (AIRail.RAILTRACK_NE_SW + AIRail.RAILTRACK_NE_SE)) ||
				...
				(AIRail.GetRailTracks(t) == (AIRail.RAILTRACK_NW_NE + AIRail.RAILTRACK_NE_SE))) {return false;} // a trailing junction which is presumably the start of a passing place. Remove and continue.
...
if (AIRoad.IsRoadTile(t) || AITile.GetOwner(t) != Me) { AITile.DemolishTile(t); }
...
if (!AIRail.IsRailTile(t) || AIRail.IsRailDepotTile(t) || AIRail.IsRailStationTile(t) || AITile.GetOwner(t) != Me) { return false; } // trying to pbuild on an unbuildable tile could take forever...
if (((AIRail.GetRailTracks(t) & track) != track) && (!FlatRail(t, 0) || !FlatRail(t, 2))) { return false; } // trying to pbuild a junction on a non-flat tile?
	 ...
	 if (!AIRail.IsRailStationTile(tt) && AIRail.GetRailTracks(tt) == AIRail.RAILTRACK_NW_SE) // station end facing direction 0
		{
		 pig.AddItem(AIMap.GetTileIndex(tg[0]+0,tg[1]-3), 0) 
			if (!AIRail.IsRailTile(AIMap.GetTileIndex(tg[0]+1,tg[1]-2))) { sig.AddItem(AIMap.GetTileIndex(tg[0]+1,tg[1]-2), 3); } 
			if (!AIRail.IsRailTile(AIMap.GetTileIndex(tg[0]-1,tg[1]-2))) { sig.AddItem(AIMap.GetTileIndex(tg[0]-1,tg[1]-2), 2); }
		}
	tt = AIMap.GetTileIndex(tg[0]+0,tg[1]+1);	
	 ...
	 if (!AIRail.IsRailStationTile(tt) && AIRail.GetRailTracks(tt) == AIRail.RAILTRACK_NE_SW) // station end facing direction 2
		{
		 pig.AddItem(AIMap.GetTileIndex(tg[0]-3,tg[1]+0), 2) 
			if (!AIRail.IsRailTile(AIMap.GetTileIndex(tg[0]-2,tg[1]+1))) { sig.AddItem(AIMap.GetTileIndex(tg[0]-2,tg[1]+1), 1); } 
			if (!AIRail.IsRailTile(AIMap.GetTileIndex(tg[0]-2,tg[1]-1))) { sig.AddItem(AIMap.GetTileIndex(tg[0]-2,tg[1]-1), 0); }
		}
	tt = AIMap.GetTileIndex(tg[0]+1,tg[1]+0);	
...
foreach (i, dir in pig) { // check if an end connection is clear
	...
	if (dir == 0) { g = AIMap.GetTileIndex(AIMap.GetTileX(i),AIMap.GetTileY(i) - 1); } 	
	if (dir == 1) { g = AIMap.GetTileIndex(AIMap.GetTileX(i),AIMap.GetTileY(i) + 1); } 	
	if (dir == 2) { g = AIMap.GetTileIndex(AIMap.GetTileX(i) - 1,AIMap.GetTileY(i)); } 	
	if (dir == 3) { g = AIMap.GetTileIndex(AIMap.GetTileX(i) + 1,AIMap.GetTileY(i)); } 
...
return [gotile, intile]; } else {return null;}
}
...
if (AITile.GetOwner(i) == Me && AIRail.IsRailTile(i)) { return false } // abort if we built through the intile - it's not navigable due to PBS.
...
if ((AIRail.GetRailTracks(j) & AIRail.RAILTRACK_NE_SW) == AIRail.RAILTRACK_NE_SW) { e2 = true; e3 = true; }
if ((AIRail.GetRailTracks(j) & AIRail.RAILTRACK_NW_SE) == AIRail.RAILTRACK_NW_SE) { e0 = true; e1 = true; }
if ((AIRail.GetRailTracks(j) & AIRail.RAILTRACK_NW_NE) == AIRail.RAILTRACK_NW_NE) { e0 = true; e2 = true; }
if ((AIRail.GetRailTracks(j) & AIRail.RAILTRACK_SW_SE) == AIRail.RAILTRACK_SW_SE) { e3 = true; e1 = true; }
if ((AIRail.GetRailTracks(j) & AIRail.RAILTRACK_NW_SW) == AIRail.RAILTRACK_NW_SW) { e0 = true; e3 = true; }
if ((AIRail.GetRailTracks(j) & AIRail.RAILTRACK_NE_SE) == AIRail.RAILTRACK_NE_SE) { e2 = true; e1 = true; }
} 

local jg = [AIMap.GetTileX(j), AIMap.GetTileY(j)];
if (AIRail.IsRailTile(AIMap.GetTileIndex(jg[0], jg[1] - 1)) && (AIRail.GetRailTracks(AIMap.GetTileIndex(jg[0], jg[1] - 1)) & AIRail.RAILTRACK_NW_SE) == AIRail.RAILTRACK_NW_SE) { e0 = true; }
if (AIRail.IsRailTile(AIMap.GetTileIndex(jg[0], jg[1] + 1)) && (AIRail.GetRailTracks(AIMap.GetTileIndex(jg[0], jg[1] + 1)) & AIRail.RAILTRACK_NW_SE) == AIRail.RAILTRACK_NW_SE) { e1 = true; }
if (AIRail.IsRailTile(AIMap.GetTileIndex(jg[0] - 1, jg[1])) && (AIRail.GetRailTracks(AIMap.GetTileIndex(jg[0] - 1, jg[1])) & AIRail.RAILTRACK_NE_SW) == AIRail.RAILTRACK_NE_SW) { e2 = true; }
if (AIRail.IsRailTile(AIMap.GetTileIndex(jg[0] + 1, jg[1])) && (AIRail.GetRailTracks(AIMap.GetTileIndex(jg[0] + 1, jg[1])) & AIRail.RAILTRACK_NE_SW) == AIRail.RAILTRACK_NE_SW) { e3 = true; }
...
if (!PBuild(i, AIRail.RAILTRACK_NW_SE)) { return false }
if (yof < 0) { e1 = true; } else { e0 = true; }
} else {
if (!PBuild(i, AIRail.RAILTRACK_NE_SW)) { return false }
if (xof < 0) { e3 = true; } else { e2 = true; }
}


if (e2 && e3) { PBuild(j, AIRail.RAILTRACK_NE_SW); }
if (e0 && e1) { PBuild(j, AIRail.RAILTRACK_NW_SE); }
if (e0 && e2) { PBuild(j, AIRail.RAILTRACK_NW_NE); }
if (e3 && e1) { PBuild(j, AIRail.RAILTRACK_SW_SE); }
if (e0 && e3) { PBuild(j, AIRail.RAILTRACK_NW_SW); }
if (e2 && e1) { PBuild(j, AIRail.RAILTRACK_NE_SE); }
