...
// find the exemplar --------------------------------------------
 ...
 if (exlist.Count() == 0) { exlist.AddList(woganlist); } // no native wagons found, restore the list of refittables.
...
foreach (l, z in locolist) {
	...
	if (wagonspeed > AIEngine.GetMaxSpeed(l) || wagonspeed == 0) { wagonspeed = AIEngine.GetMaxSpeed(l); }
...
if (!AIRail.IsRailDepotTile(depot) && !costing) {
	 if (AIRail.IsRailDepotTile(FindTrainDepot(a))) {depot = FindTrainDepot(a); }
else if (AIRail.IsRailDepotTile(FindTrainDepot(b))) {depot = FindTrainDepot(b); }
else {return null;} // no depot, oops
}
  // first, pick a loco
...
local pax = FindCargo("PASS");
local mail = FindCargo("MAIL");
if (!AIRail.IsRailDepotTile(depot)) {railbase = null} else {railbase = AIRail.GetRailType(depot);}

local ft;
if (AIGameSettings.IsValid("vehicle.freight_trains")) {ft = AIGameSettings.GetValue("vehicle.freight_trains");} else { ft = 1;}
...
// find the exemplar --------------------------------------------
 ...
 if (exlist.Count() == 0) { exlist.AddList(woganlist); } // no native wagons found, restore the list of refittables.
  ...
  // pick an exemplar: by capacity or speed, then by intro date
			...
			if (BiasBig > BiasFast) {
				exlist.Valuate(AIEngine.GetCapacity); exlist.Sort(AIList.SORT_BY_VALUE, false);
				exemplar = exlist.Begin();
				exlist.KeepValue(exlist.GetValue(exemplar));				
			} else {
 				exlist.Valuate(AIEngine.GetMaxSpeed); exlist.Sort(AIList.SORT_BY_VALUE, false);
				exemplar = exlist.Begin();
				exlist.KeepValue(exlist.GetValue(exemplar));	 
			}
  				exlist.Valuate(AIEngine.GetDesignDate);
...
//AILog.Info("Assessing " + AIEngine.GetName(w));
 ...
 {bvlist.RemoveItem(w)} // no locos, unrefits or non-rail vehicles
 }
 if (bvlist.Count() > 0) {
...
//AILog.Info("Assessing " + AIEngine.GetName(w));
 ...
 {mwlist.RemoveItem(w)} // no locos, unrefits or non-rail vehicles
 }
 if (mwlist.Count() > 0) {
...
if (AICargo.HasCargoClass(cargo, AICargo.CC_PIECE_GOODS) // add random wagons for piece goods
 ...
 if (AIEngine.GetPrice(wogan) > AICompany.GetBankBalance(Me)) {AIController.Sleep(30);}
			...
			if (AIVehicle.IsValidVehicle(newwogan)) { // newwogan still exists, it didn't get moved
					...
					if (!newwoganlisted) {
						woganlist.AddList(newwoganlist);
									if (BiasBig > BiasFast) {
									woganlist.Valuate(AIEngine.GetCapacity); woganlist.Sort(AIList.SORT_BY_VALUE, false);
								} else {
									woganlist.Valuate(AIEngine.GetMaxSpeed); woganlist.Sort(AIList.SORT_BY_VALUE, false);
								}
						newwoganlisted = true;
							}
...
if(!AIVehicle.IsValidVehicle(train)) {return null;} // oops, we sold the loco
...
if (cc == green || cc == darkgreen || cc == palegreen) { cname = ["Minty", "Percy", "Apple", "Flying Cucumber"] }
if (cc == purple || cc == mauve) { cname = ["Pinky", "Pruplethingz", "Cyclops", "Royal Purple"] }
if (cc == orange) { cname = ["Juicy", "Wildcard", "Naranja", "Big O"] }
if (cc == brown) { cname = ["Scuddles", "Bomber", "Richard", "Bull"] }
