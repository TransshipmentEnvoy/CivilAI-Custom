    ...
    foreach(dock2, z in docklist) {
        if (
            ...
            if (intersects.Count() > 0) {
                ...
                if (cost == null) {
                    AILog.Info("There are no ferries available to buy");
                    return;
                } // there are no ferries available
                cost = cost + (AIMarine.GetBuildCost(AIMarine.BT_BUOY) * 10);
                if (cost > dosh) {
                    AILog.Info("I don't have enough money to buy a ferry between " + AIStation.GetName(dock1) + " and " + AIStation.GetName(dock2) + ".");
                    return;
                }
...
function CivilAI::FerryReview() {
    ...
    foreach(veh, z in ferrylist) {
        if (AIVehicle.GetVehicleType(veh) != AIVehicle.VT_WATER || AIVehicle.GetCapacity(veh, 0) == 0) {
            ferrylist.RemoveItem(veh);
        }
    }
...
function CivilAI::BuildADock(station) {
    ...
    while ((i < s) && !(AIStation.HasStationType(station, AIStation.STATION_DOCK))) {
        ...
        for (; y >= 0 - i; y--) {
            ...
            if (AITile.IsCoastTile(trytile) && AIRoad.IsRoadTile(trytile) && !AIRoad.IsDriveThroughRoadStationTile(trytile) && AIRoad.GetNeigbourRoadCount(trytile) == 1) {
                AITile.DemolishTile(trytile)
            } // clear road spur
    ...
    if (AITile.GetSlope(tile) == AITile.SLOPE_NW) {
        ...
        if (
            ...
            if (AIMarine.BuildDock(tile, station)) {
                ...
                if (AIMarine.BuildWaterDepot(deptile2, deptile1)) {} else if (AIMarine.BuildWaterDepot(deptile4, deptile3)) {} else if (AIMarine.BuildWaterDepot(deptile5, deptile3)) {}
...
function CivilAI::Beamage(dock, oilrig) {
    ...
    if (oilrig) {
        ...
        if (dtlist.Count() != 2) {
            AILog.Info("Something went wrong assessing the dock...");
            return null;
        }
        ...
        if (copycat == null) {
            ...
            foreach(v, z in shiplist) {
                if (AIVehicle.GetVehicleType(v) != AIVehicle.VT_WATER) {
                    shiplist.RemoveItem(v);
                } else {
                    avgage = avgage + AIVehicle.GetAge(v);
                    profits = (profits + AIVehicle.GetProfitLastYear(v) + AIVehicle.GetProfitThisYear(v) - AIEngine.GetRunningCost(AIVehicle.GetEngineType(v)));
                }
            }
            ...
            if (shiplist.Count() == 0) {
                ...
                avgage = ((avgage / shiplist.Count()) / 365);
            }

            if (profits < 0) {
                AILog.Info("I won't build another ship for " + AIStation.GetName(dock1) + " -> " + AIStation.GetName(dock2) + " because the route is unprofitable.");
                return null;
            } else if (avgage < 3) {
                AILog.Info("I won't build another ship for " + AIStation.GetName(dock1) + " -> " + AIStation.GetName(dock2) + " because the current ships are too new.");
                return null;
            }
    ...
    if (ship == null) {
        ...
        if (newship = (AIVehicle.BuildVehicle(depot, ship))) {
            ...
            if (copycat != null) {
                ...
                b = buoylist.Begin();
                for (c = 0; c < o; c++) {
                    AIOrder.AppendOrder(newship, b, AIOrder.OF_NONE);
                    b = buoylist.Next();
                }
            ...
            local prefix;
            if (AIDate.GetYear(AIEngine.GetDesignDate(AIVehicle.GetEngineType(newship))) < 1950) {
                prefix = "SS ";
            } else {
                prefix = "MV ";
            }
            ...
            AILog.Info("For some reason, I couldn't build a new ship.");
            return null;
        }
