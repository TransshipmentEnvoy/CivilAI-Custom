...
class RoadPF.Cost
{
    ...
    function _set(idx, val)
    {
        ...
        switch (idx) {
            ...
            case "bus_stop":          this._main._cost_bus_stop = val; break;
    ...
    function _get(idx)
    {
        switch (idx) {
            ...
            case "bus_stop":          return this._main._cost_bus_stop;
...
function RoadPF::FindPath(iterations)
{
...
function RoadPF::_GetBridgeNumSlopes(end_a, end_b)
{
    local slopes = 0;
...
function RoadPF::_Cost(self, path, new_tile, new_direction)
{
    ...
    /* Check if the new tile is a drivethrough stop owned by this company. */
    if (AIRoad.IsDriveThroughRoadStationTile(new_tile) && AIStation.IsValidStation(AIStation.GetStationID(new_tile))) {
        cost += self._cost_bus_stop;
    }    

    return path.GetCost() + cost;
}

function RoadPF::_Estimate(self, cur_tile, cur_direction, goal_tiles)
{
...
function RoadPF::_Neighbours(self, path, cur_node)
{
...
function RoadPF::_CheckDirection(self, tile, existing_direction, new_direction)
{
    return false;
}

function RoadPF::_GetDirection(from, to, is_bridge)
{
 ...
 */
function RoadPF::_GetTunnelsBridges(last_node, cur_node, bridge_dir)
{
...
function RoadPF::_IsSlopedRoad(start, middle, end)
{
...
function RoadPF::_CheckTunnelBridge(current_tile, new_tile)
{
