...
function CivilAI::Airportz() {
    ...
    if (AIAirport.IsValidAirportType(AIAirport.AT_COMMUTER)) {
        apcost = AIAirport.GetPrice(AIAirport.AT_COMMUTER)
    } else if (AIAirport.IsValidAirportType(AIAirport.AT_SMALL)) {
        apcost = AIAirport.GetPrice(AIAirport.AT_SMALL)
    }

    local p2b = PickAircraft(0, 0);
    if (p2b != null) {
        planecost = AIEngine.GetPrice(p2b);
    } else {
        AILog.Info("No small aircraft are available.");
        return;
    }
    ...
    if (AIGameSettings.IsValid("economy.infrastructure_maintenance")) {
        infra = AIGameSettings.GetValue("economy.infrastructure_maintenance");
    }
    if (infra) {
    ...
    if (aplist.Count() > 0) {
        ...
        if (AltMethod) {
            ...
            foreach(bs, z in bslist) {
                if (AIStation.HasStationType(bs, AIStation.STATION_AIRPORT)) {
        ...
        foreach(ap2, z2 in aplist2) {
            ...
            if (airdist > MinAirRange) {
                ...
                if (vlist.Count() == 0 || AIVehicle.GetProfitLastYear(vlist.Begin()) > AIEngine.GetPrice(AIVehicle.GetEngineType(vlist.Begin())) / 4) {
                    ok = true;
                    if (AIStation.GetCargoWaiting(ap1, pc) > (vlist.Count() * 250) && AIStation.GetCargoWaiting(ap2, pc) > (vlist.Count() * 250)) {
                        cargo = pc;
                    } else if (AIStation.GetCargoWaiting(ap1, mc) > (vlist.Count() * 100) && AIStation.GetCargoWaiting(ap2, mc) > (vlist.Count() * 100)) {
                        cargo = mc;
                    } else {
                        ok = false;
...
function CivilAI::GoFlight(ap1, ap2, cargo) {
    ...
    if (acid != null) {
        ...
        if (AIVehicle.IsValidVehicle(newplane)) {
            ...
            if (cargo == FindCargo("PASS")) {
                AIGroup.MoveVehicle(Groups[4], newplane);
            } else {
                AIGroup.MoveVehicle(Groups[5], newplane);
            }
