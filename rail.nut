...
function CivilAI::CargoLine() {
    ...
    foreach(ts, z in tslist) {
        if (FindTrainDepot(ts) == null) {
            tslist.RemoveItem(ts);
            DudTerminus.AddItem(ts, 0);
        } // not a terminus station
        else if (gotile(ts) == null) {
            tslist.RemoveItem(ts);
            DudTerminus.AddItem(ts, 0);
        } // no second track space
    }
    ...
    if (tslist.Count() > 0) {
        ...
        if (dest != null && suptile != null) { // We found a suitable terminus, so build a line
            ...
            if (statest < mintrainlength) {
                trainlength = mintrainlength;
            }
            if (statest >= mintrainlength * 2) {
                trainlength = statest / 2;
                tracks = 2;
                trainstobuy = 2;
            }
            if (trainlength > maxtrainlength) {
                trainlength = maxtrainlength;
            }
            if (trainlength > destlength) {
                trainlength = destlength;
                if (!farm) {
                    trainstobuy++;
                    if (tracks == 1) {
                        tracks = 2
            ...
            } // don't build longer than the terminus
            ...
            if (src = TrainPickup(AIIndustry.GetIndustryID(suptile), s, o, cargo, tracks, trainlength)) {
                ...
                DudIndustries.AddItem(AIIndustry.GetIndustryID(suptile), 0);
            }
        }
    }
    ...
    if (go && BuildLine(dest, src, loopline)) {
        ...
        if (suptile != null && AIIndustry.GetIndustryID(suptile) != null) {
            ConnectedPInds.AddItem(AIIndustry.GetIndustryID(suptile), 0);
        }

    } else if (suptile != null && AIIndustry.GetIndustryID(suptile) != null) {
        DudIndustries.AddItem(AIIndustry.GetIndustryID(suptile), 0);
    }
...
function CivilAI::BuildLine(stat1, stat2, loopline) {
    ...
    if (goa != null && gob != null) {
        if (BuildALine([goa], [gob])) {
            ...
            if (!loopline) {
                AIRail.BuildSignal(goa[1], goa[0], AIRail.SIGNALTYPE_PBS);
            }
...
function CivilAI::BuildSmallStation(tile, face, cargo, ind, tracks, trainlength) {
    ...
    if (face < 2) { // station oriented on y axis
        if (!(AITile.IsBuildableRectangle(AIMap.GetTileIndex(tg[0] - 1, tg[1] - 3), tracks + 1, trainlength + 4))) {
            return null;
        }
        if (!NoWater(AIMap.GetTileIndex(tg[0] - 1, tg[1] - 3), tracks + 1, 7)) {
            return null;
        }
        ...
        if (!(AITile.IsBuildableRectangle(AIMap.GetTileIndex(tg[0] - 3, tg[1] - 1), 7, tracks + 1))) {
            return null;
        }
        if (!NoWater(AIMap.GetTileIndex(tg[0] - 3, tg[1] - 1), 7, tracks + 1)) {
            return null;
        }
...
function CivilAI::BuildPickup(tile, face, cargo, ind, tracks, trainlength) {
    ...
    if (face < 2) { // station oriented on y axis
        ...
        if (!NoWater(AIMap.GetTileIndex(tg[0] - 1, tg[1] - 3), tracks + 1, trainlength + 4)) {
            return null;
        }
        AITile.LevelTiles(tile, AIMap.GetTileIndex(tg[0] + tracks, tg[1] + trainlength));
        ...
        if (!(AITile.GetMaxHeight(tile) == AITile.GetMaxHeight(AIMap.GetTileIndex(tg[0] + tracks, tg[1] + trainlength))) ||
            ...
            !(AITile.GetMaxHeight(tile) == AITile.GetMaxHeight(AIMap.GetTileIndex(tg[0] + tracks, tg[1] - 2))) ||
            !(AITile.GetMaxHeight(tile) == AITile.GetMaxHeight(AIMap.GetTileIndex(tg[0] - 0, tg[1] - 1)))) {
            return null;
        }
        ...
        if (!(AITile.IsBuildableRectangle(AIMap.GetTileIndex(tg[0] - 3, tg[1] - 1), trainlength + 4, tracks + 1))) {
            return null;
        }
        if (!NoWater(AIMap.GetTileIndex(tg[0] - 3, tg[1] - 1), trainlength + 4, tracks + 1)) {
            return null;
        }
        ...
        if (!(AITile.GetMaxHeight(tile) == AITile.GetMaxHeight(AIMap.GetTileIndex(tg[0] + trainlength, tg[1] + tracks))) ||
            ...
            !(AITile.GetMaxHeight(tile) == AITile.GetMaxHeight(AIMap.GetTileIndex(tg[0] - 2, tg[1] + tracks))) ||
            !(AITile.GetMaxHeight(tile) == AITile.GetMaxHeight(AIMap.GetTileIndex(tg[0] - 1, tg[1] - 0)))) {
            return null;
        }
...
function CivilAI::BuildALine(a, b) {
    ...
    local path = false;
    local i = 0;
    local maxtime = TrainRange * 10; // increase max time
    local percount = 0;
