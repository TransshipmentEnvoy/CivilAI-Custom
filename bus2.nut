            ...
            foreach(b, z in blist) {
                ...
                while (AIOrder.IsValidVehicleOrder(b, o)) {
                    ...
                    if (AIStation.GetStationID(AIOrder.GetOrderDestination(b, o)) == bs) {
                        if (AIStation.GetCargoWaiting(bs, pax) < 50) {
                            AIOrder.SetOrderFlags(b, o, AIOrder.OF_TRANSFER + AIOrder.OF_NO_LOAD + AIOrder.OF_NON_STOP_INTERMEDIATE);
                        } else if (AIStation.GetCargoWaiting(bs, pax) > 500) {
